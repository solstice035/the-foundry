...
class TrendMatcher:
    ...
    @staticmethod
    def match_trend(
        trend: dict, history: List[dict], threshold: float = 0.6
    ) -> List[Tuple[str, dict]]:
        """
        ...
        for entry in history:
            ...
            for hist_trend in entry["trends"]:
                ...
                similarity = TrendMatcher.jaccard_similarity(
                    trend_keywords, hist_keywords
                )
...
class LifecycleCalculator:
    ...
    def calculate_trajectory(scores: List[float]) -> str:
        ...
        recent = scores[-1]
        median = (
            sorted(scores[:-1])[len(scores[:-1]) // 2] if len(scores) > 2 else scores[0]
        )
    ...
    def calculate_momentum(status: str, scores: List[float]) -> float:
        ...
        if status == "rising":
            growth_rate = (
                (recent_score - first_score) / first_score if first_score > 0 else 0
            )
            ...
            decline_rate = (
                (first_score - recent_score) / first_score if first_score > 0 else 0
            )
...
def enrich_trend_with_lifecycle(trend: dict, history: List[dict]) -> dict:
    ...
    for date, hist_trend in matches:
        engagement_history.append(
            {"date": date, "score": hist_trend["engagement_score"]}
        )

    # Add current observation
    current_date = datetime.now().strftime("%Y-%m-%d")
    current_score = trend.get("engagement_score", 0)
    engagement_history.append({"date": current_date, "score": current_score})
    ...
    days_active = (
        datetime.strptime(engagement_history[-1]["date"], "%Y-%m-%d")
...
def test_new_trend():
    ...
    assert enriched["lifecycle"]["status"] == "new", (
        f"Expected 'new', got {enriched['lifecycle']['status']}"
    )
    assert enriched["lifecycle"]["appearances"] == 1, (
        f"Expected 1 appearance, got {enriched['lifecycle']['appearances']}"
    )
    assert enriched["lifecycle"]["momentum_score"] == 1.0, (
        f"Expected momentum 1.0, got {enriched['lifecycle']['momentum_score']}"
    )
...
def test_rising_trend():
    ...
    assert enriched["lifecycle"]["status"] == "rising", (
        f"Expected 'rising', got {enriched['lifecycle']['status']}"
    )
    assert enriched["lifecycle"]["trajectory"] == "upward", (
        f"Expected 'upward', got {enriched['lifecycle']['trajectory']}"
    )
    assert enriched["lifecycle"]["appearances"] == 3, (
        f"Expected 3 appearances, got {enriched['lifecycle']['appearances']}"
    )
    assert enriched["lifecycle"]["momentum_score"] > 1.0, (
        f"Expected momentum > 1.0, got {enriched['lifecycle']['momentum_score']}"
    )
...
def test_peaked_trend():
    ...
    assert enriched["lifecycle"]["status"] == "peaked", (
        f"Expected 'peaked', got {enriched['lifecycle']['status']}"
    )
    assert enriched["lifecycle"]["trajectory"] == "downward", (
        f"Expected 'downward', got {enriched['lifecycle']['trajectory']}"
    )
    assert enriched["lifecycle"]["appearances"] == 4, (
        f"Expected 4 appearances, got {enriched['lifecycle']['appearances']}"
    )
    assert enriched["lifecycle"]["momentum_score"] < 1.0, (
        f"Expected momentum < 1.0, got {enriched['lifecycle']['momentum_score']}"
    )
...
def test_stable_trend():
    ...
    assert enriched["lifecycle"]["status"] == "stable", (
        f"Expected 'stable', got {enriched['lifecycle']['status']}"
    )
    assert enriched["lifecycle"]["trajectory"] == "flat", (
        f"Expected 'flat', got {enriched['lifecycle']['trajectory']}"
    )
    assert enriched["lifecycle"]["momentum_score"] == 1.0, (
        f"Expected momentum 1.0, got {enriched['lifecycle']['momentum_score']}"
    )
...
def test_explosive_growth():
    ...
    assert enriched["lifecycle"]["momentum_score"] >= 1.5, (
        f"Expected momentum >= 1.5, got {enriched['lifecycle']['momentum_score']}"
    )

    print(
        f"✅ PASS: Explosive growth detected with momentum {enriched['lifecycle']['momentum_score']}"
    )
...
def test_fuzzy_matching():
    ...
    assert enriched["lifecycle"]["appearances"] == 2, (
        f"Expected 2 appearances (fuzzy match), got {enriched['lifecycle']['appearances']}"
    )
...
def test_momentum_score_bounds():
    ...
    print(
        f"✅ PASS: Momentum bounds enforced (rising: {calc.calculate_momentum('rising', rising_scores)}, peaked: {calc.calculate_momentum('peaked', peaked_scores)})"
    )
...
def test_scoring_boost():
    ...
    assert rising_final > peaked_final, (
        f"Rising score ({rising_final}) should be > peaked score ({peaked_final})"
    )

    print(
        f"✅ PASS: Rising final score ({rising_final:.2f}) > Peaked final score ({peaked_final:.2f})"
    )
