...
SCHEMA_PATH = Path(__file__).parent.parent / "config" / "schemas" / "content-queue.schema.json"
...
class ContentDrafterTester:
    ...
    def check_voice_compliance(self, draft: dict) -> List[str]:
        ...
        if "x" in draft.get("platforms", {}):
            ...
            for i, tweet in enumerate(tweets, 1):
                ...
                if i == 1 and tweet.count("🚀") > 0:
                    violations.append(f"Tweet 1: Contains 🚀 (not ironic context)")
                
                # Check for hashtags (unless clearly ironic)
                if "#" in tweet and not ("#buildinpublic" in tweet.lower() and "ironic" in draft.get("notes", "").lower()):
                    violations.append(f"Tweet {i}: Contains hashtags (avoid on X)")
                
                # Check for exclamation marks (should be rare)
                if tweet.count("!") > 1:
                    violations.append(f"Tweet {i}: Multiple exclamation marks (overenthusiastic)")
        ...
        if "reddit" in draft.get("platforms", {}):
            ...
            clickbait_patterns = ["you won't believe", "what happened next", "this one trick"]
            for pattern in clickbait_patterns:
                if pattern.lower() in title.lower():
                    violations.append(f"Reddit title: Clickbait pattern '{pattern}'")
            
            # Check for required sections
            required_sections = ["**The problem:**", "**What it does:**", "**How it was built:**", "**Stack:**"]
    ...
    def check_platform_formatting(self, draft: dict) -> List[str]:
        ...
        if "x" in draft.get("platforms", {}):
            ...
            for i, tweet in enumerate(tweets, 1):
                if len(tweet) > 280:
                    issues.append(f"Tweet {i}: Exceeds 280 characters ({len(tweet)} chars)")
            ...
            if "visual" in x_draft:
                ...
                if placement and placement > len(tweets):
                    issues.append(f"X: Visual placement ({placement}) exceeds tweet count ({len(tweets)})")
        ...
        if "reddit" in draft.get("platforms", {}):
            ...
            if len(reddit_draft["title"]) > 300:
                issues.append(f"Reddit title: Exceeds 300 characters ({len(reddit_draft['title'])} chars)")
            
            # Check subreddit format
            if not reddit_draft["subreddit"].startswith("r/"):
                issues.append(f"Reddit: Invalid subreddit format '{reddit_draft['subreddit']}'")
        ...
        if "hackernews" in draft.get("platforms", {}):
            ...
            if len(hn_draft["title"]) > 80:
                issues.append(f"HN title: Exceeds 80 characters ({len(hn_draft['title'])} chars)")
    ...
    def check_concrete_numbers(self, draft: dict) -> List[str]:
        ...
        for phrase in vague_phrases:
            if phrase in full_text:
                findings.append(f"WARN: Vague claim '{phrase}' (prefer specific numbers)")
        ...
        if specifics_found >= 2:
            ...
            findings.append("WARN: No specific numbers (consider adding build time, cost, metrics)")
    ...
    def test_trend_forecast(self, forecast_data: dict) -> dict:
        ...
        print(f"   Mentions: {forecast_data.get('mention_count', 0)} (up from {forecast_data.get('previous_mentions', 0)})")
