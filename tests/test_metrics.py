...
def validate_metrics_entry(entry: Dict[str, Any]) -> List[str]:
    ...
    # Type validation
    if "type" in entry and entry["type"] not in [
        "success",
        "rejected",
        "failure",
        "partial",
    ...
    for field in numeric_fields:
        if (
            ...
            and not isinstance(entry[field], (int, float))
        ):
...
def calculate_weekly_metrics(week_start: str, week_end: str) -> Dict[str, Any]:
    ...
    # Filter to week
    week_metrics = [m for m in metrics if week_start <= m["date"] <= week_end]

    if not week_metrics:
        return None

    # Overview calculations
    builds_attempted = len(
        [m for m in week_metrics if m.get("spec_decision") == "approved"]
    )
    builds_succeeded = len([m for m in week_metrics if m["type"] == "success"])
    builds_failed = len(
        [m for m in week_metrics if m["type"] in ["failure", "partial"]]
    )
    specs_rejected = len([m for m in week_metrics if m["type"] == "rejected"])

    success_rate = (
        (builds_succeeded / builds_attempted * 100) if builds_attempted > 0 else 0
    )
    ...
    build_times = [
        m["build_duration_s"] for m in week_metrics if m["build_duration_s"] > 0
    ]
    ...
    total_cost = sum(m["cost_usd"] for m in week_metrics if m["cost_usd"] is not None)
    ...
    for source, data in sources.items():
        success_rate = (
            (data["builds"] / data["trends"] * 100) if data["trends"] > 0 else 0
        )
        avg_engagement = (
    ...
    for cat, data in categories.items():
        ...
        avg_buildability = (
            sum(data["buildability"]) / len(data["buildability"])
            if data["buildability"]
            else None
        )
        ...
        category_dist.append(
            {
                ...
                "avg_buildability": round(avg_buildability, 1)
                if avg_buildability
                else None,
            }
        )
    ...
    return {
        ...
        "sources": {"effectiveness": source_effectiveness, "failures": []},
        "categories": {"distribution": category_dist},
...
def test_weekly_aggregation():
    ...
    if week_metrics:
        print("✓ Weekly aggregation calculated:")
...
def test_backfill_data_quality():
    ...
    print(
        f"✓ Found metrics for {len([d for d in phase1_dates if d in metrics_dates])}/{len(phase1_dates)} Phase 1 builds"
    )
