...
def format_engagement_details(engagement_data):
    ...
    if repos:
        ...
        for repo in repos:
            ...
            lines.append(f"  Stats: {stats.get('stars', 0)} ⭐ | {stats.get('forks', 0)} 🍴 | {stats.get('issues', 0)} 📝")
            ...
            if notable:
                lines.append(f"  Notable issues:")
                for issue in notable[:3]:
                    lines.append(f"    #{issue['number']}: {issue['title']} (@{issue['author']})")
