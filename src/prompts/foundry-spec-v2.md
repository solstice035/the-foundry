# foundry-spec v2 — Spec Writer Task Prompt
...
2. **History:** `~/.openclaw/workspace/foundry/history.json` (for reference)
...
Check each trend for the `previously_built` flag:
- If `previously_built: true` → trend is a duplicate of a recent build or rejection
- The `duplicate_info` object contains the reason, similarity, and matched keywords

**Decision rule for duplicates:**
- **Generally REJECT** duplicates with similarity ≥ 60%
- **Consider rebuilding** only if:
  - Original build was >30 days ago (outside active dedup window)
  - New trend has significantly higher engagement (2x+ the original)
  - You can articulate a clear differentiation or improvement over the original

**Why allow exceptions?**
- Sometimes re-building with improvements is valid
- User demand may have grown significantly
- Technology may have changed (new APIs, better libraries)

**Rejection reason format for duplicates:**
```
"Duplicate of [project-name] built on [date] ([similarity]% keyword overlap). [Additional context if needed]"
```

### Step 3: Evaluate Each Candidate

For each of the top 5 trends (prioritizing non-duplicates), evaluate against ALL of these criteria:

**NOTE:** Trends may include `lifecycle` data from the Trend Researcher (if enabled):
...
- **Not flagged as duplicate** (unless exception criteria met)
...
6. **Respect dedup flags.** If Trend Scout flagged a duplicate, reject it unless you have strong justification to override. ✨ NEW in v2
...
- "Duplicate of [project-name] built on [date] ([similarity]% keyword overlap)" ✨ NEW in v2
