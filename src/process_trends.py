...
def deduplicate_trends(trends: List[Dict]) -> List[Dict]:
    ...
    for trend in trends:
        ...
        if matched_idx is None:
            trend_keywords = extract_keywords(
                trend.get("title", "") + " " + trend.get("raw_title", "")
            )
        ...
        if matched_idx is not None:
            ...
            if trend["engagement_value"] > existing["engagement_value"]:
                ...
                existing["source"] = trend[
                    "source"
                ]  # Primary source = highest engagement
...
def score_buildability(trend: Dict) -> Tuple[int, Dict[str, int], str]:
    ...
    if not (is_non_buildable or is_news_category) and any(
        kw in text for kw in diff_saturated
    ):
        diff_score = 0
...
def process_trends(input_path: str, output_dir: str) -> Dict[str, Any]:
    ...
    print(
        f"🔗 Deduplication: {len(trends)} → {len(deduped)} ({duplicates_merged} duplicates merged)"
    )
    ...
    print(
        f"🚫 Auto-filters: {len(rejected)} trends rejected, {len(filtered)} remaining"
    )
    for r in rejected:
    ...
    for trend in filtered:
        ...
        eng_normalized = normalize_engagement(
            trend["engagement_value"], trend["source"]
        )
        ...
        "trends": [
            {
                ...
                "sources": list(
                    set(
                        s["source"]
                        for s in t.get("all_sources", [{"source": t["source"]}])
                    )
                ),
    ...
    for trend in scored_trends:
        ...
        full_detail = {
            ...
            "sources": list(
                set(
                    s["source"]
                    for s in trend.get("all_sources", [{"source": trend["source"]}])
                )
            ),
            "source_urls": [
                s.get("source_url", "")
                for s in trend.get(
                    "all_sources", [{"source_url": trend.get("source_url", "")}]
                )
            ],
    ...
    print(
        f"{'Rank':<5} {'Score':<8} {'Build':<6} {'Eng':<6} {'Amp':<5} {'Source':<8} {'Title'}"
    )
    print("-" * 100)
    for idx, t in enumerate(top_15):
        sources = list(
            set(s["source"] for s in t.get("all_sources", [{"source": t["source"]}]))
        )
        source_str = "+".join(sources)
        title = t.get("raw_title", t.get("title", ""))[:55]
        print(
            f"{idx + 1:<5} {t['final_score']:<8} {t['buildability_adjusted']:<6} {t['engagement_normalized']:<6} {t['amplification_factor']:<5} {source_str:<8} {title}"
        )
...
def history_dedup_trends(
    ...
    print(
        f"✅ History dedup: {duplicates_flagged}/{len(trends_data.get('trends', []))} trends flagged as duplicates"
    )
...
if __name__ == "__main__":
    ...
    print("🏭 Epic 1.2: Trend Scout - Normalization & Scoring")
    ...
    print("\n✅ Processing complete!")
    print(
        f"   Raw: {stats['raw_count']} → Deduped: {stats['after_dedup']} → Filtered: {stats['after_filter']}"
    )
    ...
    for i, t in enumerate(stats["top_3"]):
        print(
            f"   {i + 1}. {t['title'][:70]} (score: {t['final_score']}, build: {t['buildability']}, src: {'+'.join(t['sources'])})"
        )
